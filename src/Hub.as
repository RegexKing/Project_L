package
{
	...
	public class Hub
	{
		...
		protected function fillRect(startX:int, startY:int, width:int, height:int):void {
			for (var x:int = 0; x < width; x++) {
				for (var y:int = 0; y < height; y++) {
					...
					var i:int = (startY + y) + (startX + x) * TOTAL_COLS;
					
					//FlxG.log(i);
					
					storeRoom(i); // Store the coordinates in the rooms array, to keep track of all rooms' floors
				}
			}			
		}
		 ...
		 */
		protected function storeRoom(x:int):void {
			rooms.push(x);	
		}
		 ...
		 * @return Array. The random point's coordinates, as (x, y)
		 */
		public function getRandomRoomTile():FlxPoint {
			var index:int = Math.floor(Math.random() * rooms.length);
			
			return new FlxPoint((rooms[index] % DungeonGenerator.TOTAL_ROWS) * Dungeon.TILE_SIZE, Math.floor(rooms[index] / DungeonGenerator.TOTAL_COLS) * Dungeon.TILE_SIZE);
		}
